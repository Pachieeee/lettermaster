  ...
  methods: {
    ...
    confirmDeleteSelected() {
      this.$confirm.require({
        message:      `Seguro que desea eliminar las ${this.title.plural} seleccionadas?`,
        header:       'Confirmación',
    ...
    confirmDeleteItem(item) {
      this.$confirm.require({
        message:      `¿Seguro que desea eliminar la ${this.title.singular} seleccionada?`,
        header:       'Confirmación',
        icon:         'pi pi-exclamation-triangle',
        rejectClass:  'p-button-secondary p-button=outlined',
        rejectLabel:  'Cancelar',
...
<template>
  ...
  <pv-toolbar class="mb-4">
    <template #start>
      <pv-button class="mr-2" icon="pi pi-plus" label="Nuevo" severity="success" @click="newItem"/>
      <pv-button :disabled="!selectedItems || !selectedItems.length" icon="pi pi-trash" label="Eliminar" severity="danger"
                 @click="confirmDeleteSelected"/>
    </template>
    <template #end>
      <pv-button icon="pi pi-download" label="Exportar" severity="help" @click="exportToCsv($event)"/>
    </template>
  </pv-toolbar>
